...
export default function ShareDialog({ open, documentId, ownerId, isDark, onClose }: ShareDialogProps) {
  ...
  async function loadShares() {
    setLoading(true)
    try {
      const data = await getSharesForDocument(documentId)
      setShares(data)
    } catch (err) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const share = await createShareLink(documentId, ownerId, permission, expiryHours)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className={`relative w-full max-w-md mx-4 rounded-2xl border shadow-xl overflow-hidden ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-[#e8e4dc]'}`}>
        {/* Header */}
        ...
        <div className={`px-6 py-4 border-b ${isDark ? 'border-neutral-800' : 'border-[#e8e4dc]'}`}>
          <div className="flex items-center gap-3 mb-3">
            <div className="flex-1">
              ...
              <select
                ...
                className={`w-full text-sm px-3 py-2 rounded-lg border outline-none ${isDark ? 'bg-neutral-800 text-white border-neutral-700' : 'bg-white text-[#2d2a26] border-[#e8e4dc]'}`}
              >
          ...
          <button
            ...
            disabled={creating}
            className={`w-full py-2.5 text-sm font-medium rounded-xl transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 ${isDark ? 'bg-white text-black' : 'bg-[#2d2a26] text-white'}`}
          >
                  ...
                  <div className="flex items-center gap-1 ml-2 shrink-0">
                    {!isExpired(share) && (
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors ${isDark ? 'text-neutral-500 hover:text-white hover:bg-neutral-800' : 'text-[#9c958a] hover:text-[#2d2a26] hover:bg-[#f5f2ed]'}`}
                        title="Copy link"
                      >
                        {copiedId === share.id ? (
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors ${isDark ? 'text-neutral-600 hover:text-red-400 hover:bg-neutral-800' : 'text-[#9c958a] hover:text-red-500 hover:bg-red-50'}`}
                      title="Revoke"
                    >
